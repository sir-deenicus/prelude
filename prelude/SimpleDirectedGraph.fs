...
type LabeledDirectedGraph<'a, 'b when 'a : equality and 'a : comparison>() =
    ...
    member g.GetEdge v1 v2 = maybe { let! w = edgeWeights.tryFind (struct (v1, v2))
                                     return w }
    ...
    member g.Edges =
        Hashset(g.EdgeData
                |> Seq.collect (fun (DictKV(v1, vs)) -> 
                       vs
                       |> Seq.map (fun v2 -> (v1, v2), edgeWeights.[struct (v1, v2)])
                       |> Seq.toArray)
                |> Seq.toArray)
        |> Seq.toArray 
                    ...
                    for wnode in edges.[v] do
                        if currentCut.Contains wnode.Node 
                           || currentCut.Contains v then 
                            ignore 
                            <| fs.Add (WeightPair (wnode.Weight * dir, v, wnode.Node))
                            ()
