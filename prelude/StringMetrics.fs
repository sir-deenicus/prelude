        ...
        | i -> 
            ...
            for j in 0..arr2.Length - 1 do
                let delcost, addcost, subcost =
                    oneback.[j] + 1, thisrow.[wrap j 0] + 1, 
                    oneback.[wrap j 0] + if arr1.[i] <> arr2.[j] then 1
                                         else 0
                thisrow.[j] <- [ delcost; addcost; subcost ] |> List.min
                if i > 0 && j > 0 && arr1.[i] = arr2.[j - 1] 
                   && arr1.[i - 1] = arr2.[j] && arr1.[i] <> arr2.[j] then 
                    thisrow.[j] <- min (thisrow.[j]) (twoback.[wrap j 1] + 1)
            outer thisrow oneback thisrow.[arr2.Length - 1] (i + 1)
    
    outer (Array.append (Array.init arr2.Length ((+) 1)) [| 0 |]) 
        (Array.zeroCreate (arr2.Length + 1)) (max arr1.Length arr2.Length) 0
...
let rec backtrackLCStr ((str1 : string), (str2 : string), (lcsMatrix : int [,]), 
                        wordlen1, wordlen2) =
    if wordlen1 = 0 || wordlen2 = 0 then ""
    elif str1.[wordlen1 - 1] = str2.[wordlen2 - 1] then 
        backtrackLCStr (str1, str2, lcsMatrix, (wordlen1 - 1), (wordlen2 - 1)) 
        + string str1.[wordlen1 - 1]
    else if lcsMatrix.[wordlen1, wordlen2 - 1] > lcsMatrix.[wordlen1 - 1, 
                                                            wordlen2] then 
        backtrackLCStr (str1, str2, lcsMatrix, wordlen1, (wordlen2 - 1))
    else backtrackLCStr (str1, str2, lcsMatrix, (wordlen1 - 1), wordlen2)
