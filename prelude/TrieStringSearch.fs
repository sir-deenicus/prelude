...
let dict_as_trie words =
    words |> Array.fold (fun trie w -> add siterm trie (w, 0) ()) empty_trie
...
let autocomplete take (tri: trie<char, 'a>) (fragment: string) =
    let rec rundownword taken clist ctri curword = //printfn "is word %s, %d %A" curword taken clist;
        let nlist, count =
            if is_terminal ctri then
                curword :: clist, taken + 1
            else
                clist, taken

        node_map ctri
        |> Seq.fold
            (fun (ntaken, curlist) (DictKV(letter, children)) ->
                if ntaken < take then
    ...
    let starttri = fragment |> Seq.fold (fun curtrie c -> find_subtrie c curtrie) tri
...
let search maxCost take tri word =
    ...
    let rec searchRecursive node letter currentWord word (previousRow: int[]) maxCost =
        ...
        for column = 1 to len do
            ...
            let replaceCost =
                previousRow.[column - 1] + (if word.[column - 1] <> letter then 1 else 0)
